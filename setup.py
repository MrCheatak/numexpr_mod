...
# # Fetch the version for numexpr_mod (will be put in variable `version`)
# with open(os.path.join('numexpr_mod', 'version.py')) as f:
#     exec(f.read())
major_ver = 2
minor_ver = 8
nano_ver = 0
...
def setup_package():
    ...
    if (len(sys.argv) >= 2 and
        ...
        def configuration():
            ...
            config = Configuration('numexpr_mod')
            ...
            if os.name == 'nt':
                pthread_win = ['numexpr_mod/win32/pthread.c']
            else:
                pthread_win = []
            extension_config_data = {
                'sources': ['numexpr_mod/interpreter.cpp',
                            'numexpr_mod/module.cpp',
                            'numexpr_mod/numexpr_object.cpp'] + pthread_win,
                'depends': ['numexpr_mod/interp_body.cpp',
                            'numexpr_mod/complex_functions.hpp',
                            'numexpr_mod/interpreter.hpp',
                            'numexpr_mod/module.hpp',
                            'numexpr_mod/msvc_function_stubs.hpp',
                            'numexpr_mod/numexpr_config.hpp',
                            'numexpr_mod/numexpr_object.hpp'],
                'libraries': ['m'],
            ...
            config.add_subpackage('tests', 'numexpr_mod/tests')

            #version handling
            config.get_version('numexpr_mod/version.py')
            return config
        ...
        class cleaner(clean):
            ...
            def run(self):
                ...
                try:
                    ...
                    if os.name == 'posix':
                        paths = [localpath("numexpr_mod/interpreter.so")]
                    else:
                        paths = [localpath("numexpr_mod/interpreter.pyd")]
                else:
                    paths = []
                    for suffix, _, _ in imp.get_suffixes():
                        if suffix == '.py':
                            continue
                        paths.append(localpath("numexpr_mod", "interpreter" + suffix))
                paths.append(localpath("numexpr_mod/__config__.py"))
                paths.append(localpath("numexpr_mod/__config__.pyc"))
                for path in paths:
                    try:
                        os.remove(path)
                    except Exception:
